...
double beta_c;
double beta_ini, beta_fin, delta_beta;
...
int main(int argc, char **argv) {
  ...
  /* 0.1~1.0まで幾つのbetaをとるか */
  printf("itertionNumber: ");
  scanf("%d", &iterationNumber);

  /* ある値 beta について何回のデータを作るか */
  printf("n_beta: ");
  scanf("%d", &n_beta);

  /* alpha固定でbetaを変化させていく.*/
  delta_beta = (beta_fin - beta_ini) / (double)iterationNumber;

  /*******  loop of alpha begins *****************/

  for (iter = 0; iter < iterationNumber; iter++) {

    beta = beta_ini + delta_beta * (double)iter;

    /*ある値 beta について複数回のデータを作る*/
    for (j = 0; j < n_beta; j++) {
